      ...
      <div className="mx-auto max-w-6xl">
        ...
        {loading ? (
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900 p-10 text-center">
            ...
            <p className="mt-2 text-slate-400">
              Update your profile or check back later for more suggestions.
            </p>
            <Link
              href="/profile"
              className="mt-6 inline-block rounded-lg bg-indigo-600 px-5 py-3 font-semibold hover:bg-indigo-500"
            >
              Update profile
            </Link>
          </div>
          ...
          <>
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {visibleSuggestions.map((match) => {
                ...
                return (
                  <article
                    key={match.id}
                    className="flex flex-col rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-lg"
                  >
                      ...
                      <div className="flex flex-wrap gap-2">
                        {match.sharedInterests.length > 0 ? (
                          match.sharedInterests.map((interest) => (
                            <span
                              key={interest}
                              className="rounded-full bg-indigo-500/15 px-3 py-1 text-sm text-indigo-300"
                            >
                    ...
                    <button
                      ...
                      className="mt-6 w-full rounded-lg bg-indigo-600 px-4 py-3 font-semibold text-white transition hover:bg-indigo-500 disabled:cursor-not-allowed disabled:bg-slate-700 disabled:text-slate-400"
                    >
                      {requestSent
                        ? 'Request sent'
                        : isRequesting
            ...
            <div className="mt-8 flex items-center justify-center gap-4">
              <button
                ...
                className="rounded-lg border border-slate-700 px-4 py-2 text-sm text-slate-300 hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-40"
              >
                Previous
              </button>
              ...
              <button
                type="button"
                disabled={suggestions.length < 10 || loading}
